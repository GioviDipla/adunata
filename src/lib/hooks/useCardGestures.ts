 ...
 * viewer, browser). Encapsulates the long-press detection plus the
 * user-configurable control inversion, so every consumer behaves
 * identically and stays in sync with the explanation shown on /about.
 *
 * Default mapping:
 ...
 * The two are independent (per the user's preference choice).
 *
 * Spread the returned `rootProps` on the interactive root element. Inner
 * controls (buttons, badges) must call `stopPropagation` so they don't also
 * trigger the root gesture.
 */
export function useCardGestures({
  ...
  // on pointerdown so we can route the tap to the mobile mapping.
